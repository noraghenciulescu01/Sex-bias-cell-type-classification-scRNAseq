...
for (assay_name in assayNames(pb)){
  ...
  pb_subset_fil <- pb_subset_fil[rowSums(2^assay(pb_subset_fil, assay_name) - 1 > 1) >= 10, ]
  
  # DS analysis
  df <- tryCatch({
    res <- pbDS(pb_subset_fil, verbose = FALSE)
    # save res to file
    saveRDS(res, file = paste0("individualDS/result_", assay_name, ".rds"))
...
# save results
saveRDS(final_results, "final_DS_results.rds")